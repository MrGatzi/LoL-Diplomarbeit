MainController.controller('RecentMatchesCRL',['$scope','$window','$routeParams','$http', function($scope,$window,$routeParams,$http) {
	$scope.Overview={
			'SumName':0 ,
			'SumInfo':0 ,
			'GameInfo':0,
	...
	*/
	$scope.GameClick = function(GameId) {
		...
		success(function(data, status, config) {
			$window.location.href = 'http://www.matchupleague.com/#/MatchDetails/'+$scope.message.Name+'/'+$scope.message.Server+'/'+$scope.message.SumGames.games[GameId].gameId;
			})
		.error(function(data, status, headers, config) {
			alert("fail");
		});

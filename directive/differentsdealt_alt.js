MainController.directive('differentsdealt', function($rootScope) {
    return {
        restrict: 'AE',
        scope: {
            'leftPlayer': '=',
        ...
        link: function(scope, element, attrs) {
            ...
            var chartLeft = d3.select(".chartLeftdifferentsdealt")
                .attr("width", chartWidth)
                .attr("height", chartHeight);

            var chartRight = d3.select(".chartRightdifferentsdealt")
            ...
            scope.$watch('leftPlayer', function(newValue, oldValue) {
                if (newValue != oldValue) {
                    var data = {
                        ...
                        series: [{
                            label: 'Physical',
                            values: [scope.leftPlayer.physicalDamageDealt]
                        }, {
                            label: 'Magical',
                            values: [scope.leftPlayer.magicDamageDealt]
                        }, {
                            label: 'True',
                            values: [scope.leftPlayer.trueDamageDealt]
                        }, ]
                    };
                    ...
                    var x = d3.scale.linear()
                        .domain([0, scope.leftPlayer.highestChartdealt])
                        .range([0, chartWidth]);
            ...
            scope.$watch('rightPlayer', function(newValue, oldValue) {
                if (newValue !== oldValue) {
                    ...
                    var chartHeight = barHeight * zippedData.length + gapBetweenGroups * data.labels.length;
                    var x = d3.scale.linear()
                        .domain([0, scope.rightPlayer.highestChartdealt])
                        .range([0, chartWidth]);

MainController.directive('differnetstaken', function($rootScope) {
    return {
        restrict: 'AE',
        ...
        link: function(scope, element, attrs) {
            ...
            var chartLeft = d3.select(".chartLeftDiffernetsTaken")
                .attr("width", chartWidth)
                .attr("height", chartHeight);

            var chartRight = d3.select(".chartRightDiffernetsTaken")
            ...
            scope.$watch('leftPlayer', function(newValue, oldValue) {
                if (newValue != oldValue) {
					...
					var showX=d3.scale.linear()
                        .domain([0, scope.leftPlayer.highestCharttaken])
                        .range([chartWidth, 0]);
            ...
            scope.$watch('rightPlayer', function(newValue, oldValue) {
                if (newValue !== oldValue) {
                    ...
                    var data = {
                        ...
                        series: [{
                            label: 'Physikl',
                            values: [scope.rightPlayer.physicalDamageTaken]
                        }, {
                            label: 'Magical',
                            values: [scope.rightPlayer.magicDamageTaken]
                        }, {
                            label: 'True',
                            values: [scope.rightPlayer.trueDamageTaken]
                        }, ]
                    };
                    // Zip the series data together (first values, second values, etc.)
                    ...
                    var chartHeight = barHeight * zippedData.length + gapBetweenGroups * data.labels.length;
                    var x = d3.scale.linear()
                        .domain([0, scope.rightPlayer.highestCharttaken])
                        .range([0, chartWidth]);
